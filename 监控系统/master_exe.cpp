...
/*
 * __VA_ARGS__是一个可变参数的宏，实现思想是宏定义中参数
 * 列表的最后一个参数为省略号。
 * 
*/
#ifdef DEBUG
...
char* ip_log_dir (char *ip) {
    ...
    //access(filename, 0)判断文件夹是否存在，返回值为0存在，-1不存在
...
//单独的可以随时接收任何一个产生warning信息的主机的信息的一个线程。
