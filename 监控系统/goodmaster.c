...
int get_filename (int bit, int para_num, char *filename) {
    switch (bit) {
        case 100: {
            sprintf(filename, "~/Code/监控系统/log/cpuLog/%d.cpu.log", para_num);
        }break;
        case 101: {
            sprintf(filename, "~/Code/监控系统/log/diskLog/%d.mem.log", para_num);
        }break;
        case 102: {
            sprintf(filename, "~/Code/监控系统/log/memLog/%d.disk.log", para_num);
        }break;
        case 103: {
            sprintf(filename, "~/Code/监控系统/log/sysLog/%d.sysinfo.log", para_num);
        }break;
        case 104: {
            sprintf(filename, "~/Code/监控系统/log/userLog/%d.user.log", para_num);
        }break;
        case 105: {
            sprintf(filename, "~/Code/监控系统/log/procLog/%d.proc.log", para_num);
        }break;
        default : {
            printf("bit error!\n");
            break;
