...
void *alt_func (void *argv) {
    ...
    int port = 7999;
    alter_listen = create_socket(port);
    while (1) {
        printf("alter_addr\n");
...
void *func(void *argv) {
    ...
    while (q) {
        ...
        char *ipLogName = (char *)malloc(sizeof(char) * 50); 
        sprintf(ipLogName, "./log/%dIP%s_Log", para->num, inet_ntoa(q->addr.sin_addr));
        if (access(ipLogName, 0) == -1) {
            char *command = (char *)malloc(sizeof(char) * 50);
            sprintf(command, "mkdir %s", ipLogName);
            printf("%s", command);
        ...
        while ((len = recv(sockfd, &bit, 4, 0)) > 0) {
            ...
            get_filename(bit, ipLogName, filename);
            printf("%s %d\n", filename, para->num);
            //数据连接
            sleep(5);
...
int main() {
    ...
    if (pthread_create(&talter, NULL, alt_func, NULL) == -1) {
        printf("error\n");
        exit(1);
    }
