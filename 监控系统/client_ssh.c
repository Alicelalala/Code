...
int get_message_filename (int bit, char *filename) {
            switch (bit) {
                case 100: {
                    sprintf(filename, "~/shell/cpu.log");
                } break;
                case 101: {
                    sprintf(filename, "~/shell/mem.log");
                } break;
                case 102: {
                    sprintf(filename, "~/shell/disk.log");
                } break;
                case 103: {
                    sprintf(filename, "~/shell/sysinfo.log");
                } break;
                case 104: {
                    sprintf(filename, "~/shell/user.log");
                } break;
                case 105: {
                    sprintf(filename, "/shell/proc.log");
                } break;
                default : {
                    printf("bit error\n");
                    return 0;
